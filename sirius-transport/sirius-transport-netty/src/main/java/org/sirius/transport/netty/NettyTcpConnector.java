...
public class NettyTcpConnector extends NettyConnector {
	...
	protected void setOptions() {
		...
		WriteBufferWaterMark waterMark = createWriteBufferWaterMark(child.getWriteBufferLowWaterMark(),
				child.getWriteBufferHighWaterMark());
	...
	protected SocketChannelProvider.SocketType socketType() {
		...
		if (isNative && NativeSupport.isNativeKQueueAvailable()) {
			// netty provides the native socket transport for BSD systems such as MacOS
			// using JNI.
